...
**Last updated:** 2026-05-16 · **Branch:** `feature` · **Last commit:** `4f2885e` — F8.2 backend (sales reporting). Uncommitted: F8.3 backend slice (section P&L — POS revenue + COGS + wastage cost per section).

**▶ RESUME POINT:** Next slice is **F8.4 — Production yield + wastage report standalone**. The variance side already shipped in F7.4 (`/api/v1/reports/production-variance`); F8.4 wraps a wastage-only summary (per section, by category, with cost) for the chef-manager dashboard. Then F8.5 (gift card outstanding liability), F8.6 (layby ageing), F8.7 (customer / supplier statements), F8.8 (VAT return export). F8.10 WMS out of scope; F8.9 biometric defers. Deferred production follow-ups: custom-production no-BOM path (P2), PRE_ORDER ↔ production auto-wiring.

**Progress:** ~96% of MVP slices complete (50 of 52 — Phases 0-4 + F5.1 + F5.2 + F5.3 + F5.4 backend + F5.5 backend + F5.6 backend + F5.7 backend + F5.8 backend + F5.9 backend + F5.10 backend + F6.1 backend + F6.2 backend + F6.3 backend + F7.1 backend + F7.2 backend + F7.3a backend + F7.3b backend + F7.3c backend + F7.4 backend + F7.5 backend + F8.1 backend + F8.2 backend + F8.3 backend done; F8.4-F8.8 reporting remain).
