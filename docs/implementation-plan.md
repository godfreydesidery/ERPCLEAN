...
**Last updated:** 2026-05-16 · **Branch:** `feature` · **Last commit:** `8c78fb7` — F8.5 backend (gift-card outstanding liability). Uncommitted: F8.6 backend slice (layby ageing — open `customer_order` rows bucketed by days-since-create with per-type top-line balances + drill-down).

**▶ RESUME POINT:** Next slice is **F8.7 — Customer / supplier statements** (US-RPT-007 — per-party AR/AP statement listing invoices, receipts/payments, and outstanding rolling balance over a window). Then F8.8 (VAT return export). F8.10 WMS out of scope; F8.9 biometric defers. Deferred production follow-ups: custom-production no-BOM path (P2), PRE_ORDER ↔ production auto-wiring.

**Progress:** Phases 0-7 backend complete; Phase 8 reporting is F8.1..F8.6 done — F8.7 + F8.8 remain. Web UI deliverables remain on several slices and Phase-0 test debt is still outstanding — see "Pending across all of Phase 0" + per-slice "(web … deferred)" notes.
