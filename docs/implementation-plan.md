...
**Last updated:** 2026-05-16 · **Branch:** `feature` · **Last commit:** `607f071` — F8.4 backend (production wastage rollup). Uncommitted: F8.5 backend slice (gift-card outstanding liability — per-currency rollup + per-(status, currency, branch) drill-down).

**▶ RESUME POINT:** Next slice is **F8.6 — Layby / pre-order ageing** (US-RPT-014 — `customer_order` rows in open states bucketed by days-since-create or days-to-reserved_until). Then F8.7 (customer / supplier statements), F8.8 (VAT return export). F8.10 WMS out of scope; F8.9 biometric defers. Deferred production follow-ups: custom-production no-BOM path (P2), PRE_ORDER ↔ production auto-wiring.

**Progress:** Phases 0-7 backend complete; Phase 8 reporting is F8.1 + F8.2 + F8.3 + F8.4 + F8.5 done — F8.6 + F8.7 + F8.8 remain. Web UI deliverables remain on several slices and Phase-0 test debt is still outstanding — see "Pending across all of Phase 0" + per-slice "(web … deferred)" notes.
