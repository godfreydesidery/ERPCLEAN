...
**Last updated:** 2026-05-16 · **Branch:** `feature` · **Last commit:** `f5143ec` — F7.3a backend (production BOM authoring). Uncommitted: F7.3b backend slice (production batch — `production_batch` + `production_consumption` + `production_output` entities, plan with BOM explosion + reservation, start flips reservation to PROD_CONSUME, post-output writes PROD_OUTPUT + creates stock_batch for batch-tracked outputs, idempotent re-post, yield hard-reject).

**▶ RESUME POINT:** next slice is **F7.3c — Production batch lifecycle states + wastage + custom production + conversion** (covers the OUTPUT_HOT_DISPLAY → OUTPUT_COLD_DISPLAY → OUTPUT_DISCOUNTED → OUTPUT_DONATED / OUTPUT_WRITE_OFF → CLOSED transitions with compensating stock moves for the terminal donate/write-off states, `production_wastage` table + recording endpoint with mandatory category + reason, ad-hoc no-BOM custom production batches, `conversion` table for one-shot item→item transforms, optional operator-override of consumption actual_qty at post-output, PRE_ORDER customer-order consumer wiring `ProductionOutputPosted.v1` → `OrderReady.v1`). Then **F7.4** (production wastage report standalone), **F7.5** (EOD orchestration), Phase 8 reporting. F7.3b backend (added `production_batch` + `production_consumption` + `production_output` tables via `V52` + per-dialect sequences via `V52_1`; `V53` seeds `PROD.MANAGE_BATCH` / `PROD.READ_BATCH` permissions; `BomExplosionService` recursively walks `bom_line.sub_bom_id` pointers down to raw materials, scales by `plannedQty / bom.output_qty` and bumps each line by `wastage_pct`, aggregates per `input_item_id` and rejects sub-BOMs not in ACTIVE status; `ProductionBatchServiceImpl` orchestrates plan / start / post-output / cancel. Plan runs the explosion, pre-flights availability via `StockReservationService.available` (one clean 422 if any material short), then atomically reserves per material and writes one `production_consumption` row per exploded material at the planned qty; status = PLANNED, lifecycle_state = PLANNED. Start releases each reservation, posts a PROD_CONSUME outbound `stock_move` via `StockMoveService.post` (which inherits the DayGuard + moving-average cost path), captures `unit_cost` and `actual_qty = planned_qty` on each consumption row; status = IN_PROGRESS, lifecycle_state = IN_PROGRESS. Post-output computes total consumption cost = Σ(actual_qty × unit_cost), validates yield against `orbix.production.yield-hard-reject-multiple` (default 2.0× planned hard 422), derives a uniform per-output `unit_cost = total_cost / total_output_qty`, then per output line creates a `stock_batch` via `StockBatchService.createBatch` when the output item is batch-tracked (rejects missing batchNo), posts an inbound PROD_OUTPUT `stock_move` (with `batchId` stamped when applicable), and writes a `production_output` row carrying `is_pack_by_weight` + `is_primary` + `manufactured_at` + `expiry_at`. Re-post of a COMPLETED batch is a silent no-op (idempotency). Cancel allowed only from PLANNED — releases each reservation; once IN_PROGRESS use the lifecycle write-off path (F7.3c). Per-branch unique `number` auto-generated as `BATCH-BR{branchId}-{epoch-suffix}` when omitted. Events: `ProductionBatchPlanned.v1` / `ProductionBatchStarted.v1` / `ProductionConsumePosted.v1` / `ProductionOutputPosted.v1` / `ProductionBatchCancelled.v1`. `ProductionBatchController` at `/api/v1/production-batches` with `POST` (plan), `GET` (branch / section / status filters), `GET /{id}`, `POST /{id}/{start|post-output|cancel}`. New config keys `orbix.production.yield-hard-reject-multiple` + `.yield-soft-warn-fraction`) is done.

**Progress:** ~83% of MVP slices complete (43 of 52 — Phases 0-4 + F5.1 + F5.2 + F5.3 + F5.4 backend + F5.5 backend + F5.6 backend + F5.7 backend + F5.8 backend + F5.9 backend + F5.10 backend + F6.1 backend + F6.2 backend + F6.3 backend + F7.1 backend + F7.2 backend + F7.3a backend + F7.3b backend done; F7.3c + F7.4 + F7.5 + Phase 8 reporting remain).
...
**Next slice (start here):** **F7.3c** — Production batch lifecycle states + wastage + custom production + conversion + PRE_ORDER consumer wiring. Then F7.4 (production wastage standalone), F7.5 (EOD orchestration), Phase 8 reporting. F3.5 (vendor return) deferred to Phase 8; F4.1 (quotation) skipped; Flutter POS deferred; F5.10 PDF + object-storage upload deferred to a follow-on infra slice; F6.2 per-currency close variance deferred (needs `declaredCashByCurrency` on the close request); F7.2 web `/orders` + Flutter at-till collection + PRE_ORDER `ProductionOutputPosted.v1` consumer (replaces the manual `/ready` endpoint) wait on F7.3. Phase-0 test debt still outstanding.
