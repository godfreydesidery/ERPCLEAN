...
**Last updated:** 2026-05-16 · **Branch:** `feature` · **Last commit:** `284959c` — F7.2 backend (customer orders). Uncommitted: F7.3a backend slice (production BOM — `bom` + `bom_line` tables, DRAFT/ACTIVE/RETIRED lifecycle, sub-recipe lines, versioning, cycle detection).

**▶ RESUME POINT:** next slice is **F7.3b — Production batch: plan + start + record output** (covers `production_batch` + `production_consumption` + `production_output` entities, PLANNED → IN_PROGRESS state machine, sub-BOM explosion at plan time, material reservation via `StockReservationService`, start flips reservation → `PROD_CONSUME` moves, post-output emits `PROD_OUTPUT` moves + creates `stock_batch` for batch-tracked outputs + computes per-output unit cost from consumption sum, idempotent post-output by Idempotency-Key, yield-variance soft warning / hard reject). Then **F7.3c** (lifecycle states HOT_DISPLAY → COLD_DISPLAY → DISCOUNTED → DONATED/WRITE_OFF → CLOSED + wastage with category + reason + custom-production / conversion paths), **F7.4** (production wastage standalone), **F7.5** (EOD orchestration), Phase 8 reporting. F7.3a backend (added `bom` + `bom_line` tables via `V50` + per-dialect sequences via `V50_1`; `V51` seeds `PROD.MANAGE_BOM` / `PROD.READ_BOM` permissions; `Bom` entity with DRAFT → ACTIVE → RETIRED lifecycle; `BomLine` carries either `inputItemId` (material) or `subBomId` (sub-recipe) — exactly one via `@AssertTrue` on the request DTO; `BomServiceImpl` orchestrates create / patch (DRAFT only) / activate (runs BFS cycle check + auto-retires prior ACTIVE at `validFrom - 1`) / retire / version (clones the ACTIVE row + its lines into a new DRAFT at N+1); per-output-item `(output_item_id, version)` UNIQUE; events `BomDefined.v1` / `BomPatched.v1` / `BomActivated.v1` / `BomRetired.v1` / `BomVersioned.v1`; `BomController` exposes `POST /api/v1/boms` + `GET` (filterable by sectionId / outputItemId / status) + `GET/PATCH /{id}` + `POST /{id}/{activate|retire|version}`) is done.

**Progress:** ~81% of MVP slices complete (42 of 52 — Phases 0-4 + F5.1 + F5.2 + F5.3 + F5.4 backend + F5.5 backend + F5.6 backend + F5.7 backend + F5.8 backend + F5.9 backend + F5.10 backend + F6.1 backend + F6.2 backend + F6.3 backend + F7.1 backend + F7.2 backend + F7.3a backend done; F7.3b + F7.3c + F7.4 + F7.5 + Phase 8 reporting remain).
...
**Next slice (start here):** **F7.3b** — Production batch (plan + start + record output). Then F7.3c (lifecycle states + wastage + custom production + conversion), F7.4 (production wastage standalone), F7.5 (EOD orchestration), Phase 8 reporting. F3.5 (vendor return) deferred to Phase 8; F4.1 (quotation) skipped; Flutter POS deferred; F5.10 PDF + object-storage upload deferred to a follow-on infra slice; F6.2 per-currency close variance deferred (needs `declaredCashByCurrency` on the close request); F7.2 web `/orders` + Flutter at-till collection + PRE_ORDER `ProductionOutputPosted.v1` consumer (replaces the manual `/ready` endpoint) wait on F7.3. Phase-0 test debt still outstanding.
