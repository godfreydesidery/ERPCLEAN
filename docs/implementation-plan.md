...
**Last updated:** 2026-05-16 · **Branch:** `feature` · **Last commit:** `ab62656` — F7.3b backend (production batch plan / start / post-output). Uncommitted: F7.3c backend slice (production lifecycle states + wastage + close — advance-lifecycle endpoint, terminal OUTPUT_DONATED / OUTPUT_WRITE_OFF runs `StockBatchService.recallBatch` write-off, `production_wastage` table + recording endpoint, close endpoint for terminal CLOSED state).

**▶ RESUME POINT:** F7.3 production module is now P1-complete. Next slice is **F7.4 — Production wastage report + conversions** (standalone variance report joining `production_batch` planned vs `production_output` actual + `production_wastage` rows for the operator's per-section yield dashboard; `conversion` table + endpoint for one-shot non-BOM item→item transforms emitting paired PROD_CONSUME + PROD_OUTPUT moves in one transaction). Then **F7.5** (EOD orchestration), Phase 8 reporting. Deferred follow-ups in production: custom-production path (no-BOM batches with operator-supplied consumption + output lines — P2 in QA), PRE_ORDER ↔ production auto-wiring (orders module currently uses the manual `/orders/{id}/ready` endpoint; consumer of `ProductionOutputPosted.v1` to flip the source order to READY needs a `source_order_id` column on `production_batch` and a producer hook on `OrderDepositPaid.v1`).

**Progress:** ~87% of MVP slices complete (45 of 52 — Phases 0-4 + F5.1 + F5.2 + F5.3 + F5.4 backend + F5.5 backend + F5.6 backend + F5.7 backend + F5.8 backend + F5.9 backend + F5.10 backend + F6.1 backend + F6.2 backend + F6.3 backend + F7.1 backend + F7.2 backend + F7.3a backend + F7.3b backend + F7.3c backend done; F7.4 + F7.5 + Phase 8 reporting remain).
