...
**Last updated:** 2026-05-16 · **Branch:** `feature` · **Last commit:** `f1ac744` — F5.7 backend (gift-card tender at POS). Uncommitted: F7.2 backend slice (layby + pre-order — `customer_order` aggregate, state machine, stock reservation, payments, cancel-with-refund-policy, daily expiry job).

**▶ RESUME POINT:** next slice is **F7.3 — Production: BOM + batch lifecycle** (XL — split into F7.3a/b/c). Then F7.4 (production wastage), F7.5 (EOD orchestration), Phase 8 reporting. F7.2 backend (added `customer_order` / `customer_order_line` / `customer_order_payment` tables via `V48` + sequences via `V48_1`; `V49` seeds `ORDER.MANAGE` / `ORDER.COLLECT` / `ORDER.READ`; new `StockReservationService` port + impl writing `move_type = RESERVED` rows that bump `item_branch_balance.qty_reserved` without touching `qty_on_hand`; `CustomerOrder` entity with the DRAFT → RESERVED → DEPOSIT_PAID → PARTIALLY_PAID → READY → COLLECTED state machine + CANCELLED / EXPIRED branches; `CustomerOrderServiceImpl` orchestrates create / patch / reserve / pay / cancel / markReady / collect / expiry-job; payment posts route through `CashLedgerService` for CASH / BANK_TRANSFER / MOBILE_MONEY / CHEQUE (CashRefType `OrderPayment`, GlCategory `ORDER_DEPOSIT`) and through `GiftCardService.redeem` for GIFT_CARD method (idempotency ref = `CustomerOrderPayment.id`); LAYBY collect releases reservation then writes SALE moves via `StockMoveService.post`; PRE_ORDER skips reservation and uses a manual `/ready` endpoint until F7.3 lands the production-output consumer; daily `CustomerOrderExpiryJob` at `orbix.orders.expiry-cron` 00:15 UTC releases reservation + forfeits paid amount; cancel-refund policy gated by `orbix.orders.cancel-refund-window-days` (default 7 days — within window refunds prior payments on original tender; outside window forfeits); per-order idempotency via UNIQUE `(customer_order_id, idempotency_key)`; batch-tracked items rejected at collection; new `CashRefType` constants `OrderPayment` + `OrderRefund`) is done. Phase 7 module is now LAYBY backend-complete (PRE_ORDER production trigger + `OrderReady.v1` consumer wait on F7.3).

**Progress:** ~79% of MVP slices complete (41 of 52 — Phases 0-4 + F5.1 + F5.2 + F5.3 + F5.4 backend + F5.5 backend + F5.6 backend + F5.7 backend + F5.8 backend + F5.9 backend + F5.10 backend + F6.1 backend + F6.2 backend + F6.3 backend + F7.1 backend + F7.2 backend done; F7.3-F7.5 + Phase 8 reporting remain).
...
**Next slice (start here):** **F7.3** — Production: BOM + batch lifecycle (XL — split into F7.3a BOM + sub-recipes, F7.3b plan + start + record output, F7.3c lifecycle + wastage). Then F7.4 (production wastage standalone), F7.5 (EOD orchestration), Phase 8 reporting. F3.5 (vendor return) deferred to Phase 8; F4.1 (quotation) skipped; Flutter POS deferred; F5.10 PDF + object-storage upload deferred to a follow-on infra slice; F6.2 per-currency close variance deferred (needs `declaredCashByCurrency` on the close request); F7.2 web `/orders` + Flutter at-till collection + PRE_ORDER `ProductionOutputPosted.v1` consumer (replaces the manual `/ready` endpoint) wait on F7.3. Phase-0 test debt still outstanding.
