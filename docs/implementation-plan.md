...
**Last updated:** 2026-05-16 · **Branch:** `feature` · **Last commit:** `e2377d1` — F8.6 backend (layby ageing). Uncommitted: F8.7 backend slice (customer + supplier statements — chronological AR/AP with opening balance carried forward, period debits/credits, running balance).

**▶ RESUME POINT:** Next slice is **F8.8 — VAT return export** (US-NFR-COMP-001 — per-period VAT input + output rollup with line-item breakdown for the tax filing). F8.10 WMS out of scope; F8.9 biometric defers. Deferred production follow-ups: custom-production no-BOM path (P2), PRE_ORDER ↔ production auto-wiring.

**Progress:** Phases 0-7 backend complete; Phase 8 reporting is F8.1..F8.7 done — F8.8 (VAT return export) remains. Web UI deliverables remain on several slices and Phase-0 test debt is still outstanding — see "Pending across all of Phase 0" + per-slice "(web … deferred)" notes.
...
**Next slice (start here):** **F8.8** — VAT return export (per-period VAT input + output rollup with line-item breakdown for the tax filing). Deferred production follow-ups: custom-production path (P2), PRE_ORDER ↔ production auto-wiring. F3.5 (vendor return) deferred to Phase 8; F4.1 (quotation) skipped; Flutter POS deferred; F5.10 PDF + object-storage upload deferred to a follow-on infra slice; F6.2 per-currency close variance deferred (needs `declaredCashByCurrency` on the close request); F7.2 web `/orders` + Flutter at-till collection + PRE_ORDER `ProductionOutputPosted.v1` consumer (replaces the manual `/ready` endpoint) wait on a follow-up. Phase-0 test debt still outstanding.
