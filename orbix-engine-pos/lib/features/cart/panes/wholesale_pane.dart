...
/// right. Cashier types the quantity, then taps an item; the item is added
/// with that quantity in one action. Customer required for credit / pricing
/// tier resolution.
...
class _WholesalePaneState extends ConsumerState<WholesalePane> {
  ...
  void _add(MockItem item) {
    ref.read(cartProvider.notifier).addItem(item, qty: _qty);
    setState(() => _qtyText = '1');
  }
  ...
  Widget build(BuildContext context) {
    ...
    final filtered = _query.isEmpty
        ? mockItems
        : mockItems
            .where((i) => i.code.toLowerCase().contains(_query.toLowerCase()) || i.name.toLowerCase().contains(_query.toLowerCase()))
            .toList();
                              ...
                              Expanded(
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    Text(
                                      '${money(item.price)} / ${item.uom}  ·  Tier B (−5%) eligible',
