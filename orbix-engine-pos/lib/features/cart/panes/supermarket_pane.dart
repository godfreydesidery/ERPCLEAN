...
/// extra panels — items only appear in the table.
...
class _SupermarketPaneState extends ConsumerState<SupermarketPane> {
  ...
  void _addFromSearch(MockItem item) {
  ...
  Widget build(BuildContext context) {
    ...
    // Right-side reserved column widths (Qty + UnitPrice + Total + Remove)
    // so the dropdown can position itself flush with the Item-name column.
      ...
      child: Container(
        ...
        child: Stack(
          ...
          children: [
            ...
            // Combo-dropdown — positioned under the Item-name cell only,
            // floats above the data rows below.
  ...
  // ---------------------------------------------------------------------
  // Combo-dropdown — overlaid below the Item-name cell only (not the whole
  // row). Sits on top of the data rows like a standard autocomplete popup.
  // ---------------------------------------------------------------------
  Widget _suggestionsDropdown(ThemeData theme, List<MockItem> matches) {
                ...
                itemBuilder: (_, i) {
                  ...
                  return InkWell(
                    ...
                    child: Container(
                      decoration: BoxDecoration(
                        border: Border(
                          bottom: BorderSide(color: theme.colorScheme.outlineVariant.withValues(alpha: 0.25)),
                        ),
                      ),
      ...
      child: Row(
        children: [
          ...
          _cell(theme,
              ...
              child: _qtyStepper(context, ref, theme)),
          _cell(theme,
              width: wPrice,
