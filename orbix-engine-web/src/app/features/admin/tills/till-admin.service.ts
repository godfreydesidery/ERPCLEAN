...
export class TillAdminService {
  ...
  listTills(branchId: number | null): Observable<Till[]> {
  ...
  closeSession(id: number, request: CloseTillSessionRequest): Observable<TillSession> {
    return unwrap(this.http.post<ApiResponse<TillSession>>(
      `${this.base}/till-sessions/${id}/close`, request
    ));
  }

  reconcileSession(id: number): Observable<TillSession> {
    return unwrap(this.http.post<ApiResponse<TillSession>>(
