...
| **System Administrator** | Web ERP | Manage users, roles, permissions, biometrics, till assignments, company profile, VAT groups. |
...
- Role-based access control with hierarchical permissions (Permission → Role → User). **MVP**
...
- Role + permission check on **every** endpoint, not just by URL.
